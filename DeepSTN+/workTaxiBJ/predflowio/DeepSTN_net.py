    ...
    if 1:
        ...
        if P_N == 14:
            T_x = Concatenate(axis=1)(
                [T_x0, T_x1, T_x2, T_x3, T_x4, T_x5, T_x6, T_x7, T_x8, T_x9, T_x10, T_x11, T_x12, T_x13])
        if P_N == 15:
            T_x = Concatenate(axis=1)(
                [T_x0, T_x1, T_x2, T_x3, T_x4, T_x5, T_x6, T_x7, T_x8, T_x9, T_x10, T_x11, T_x12, T_x13, T_x14])
        if P_N == 16:
            T_x = Concatenate(axis=1)(
                [T_x0, T_x1, T_x2, T_x3, T_x4, T_x5, T_x6, T_x7, T_x8, T_x9, T_x10, T_x11, T_x12, T_x13, T_x14, T_x15])
...
class Arranger(keras.layers.Layer):
...
def DeepSTN(H=21, W=12, channel=2,  # H-map_height W-map_width channel-map_channel
            ...
            P_N=6, T_F=28, PT_F=6, T=48,  # P_N-poi_number T_F-time_feature PT_F-poi_time_feature T-T_times/day
            drop=0,
    ...
    # Arranger [
    c_len, p_len, t_len = c_input.shape[1], p_input.shape[1], t_input.shape[1]
    out = keras.layers.Concatenate(axis=1)([c_input, p_input, t_input])

    arranger = Arranger(H, W)
    c_input, p_input, t_input = arranger(c_input), arranger(p_input), arranger(t_input)
    # TODO: the next sentence is solution 2
    # out = arranger(out)
    #
    # def split_layer(out, begin_idx, end_idx):
    #     return out[:, begin_idx: end_idx]
    # c_input = Lambda(split_layer, arguments={'begin_idx': 0, 'end_idx': c_len})(out)
    # p_input = Lambda(split_layer, arguments={'begin_idx': c_len, 'end_idx': c_len+p_len})(out)
    # t_input = Lambda(split_layer, arguments={'begin_idx': c_len+p_len, 'end_idx': c_len+p_len+t_len})(out)
    # # c_input, p_input, t_input = out[:, :c_len], out[:, c_len:c_len+p_len], out[c_len:c_len+p_len:]
    ...
    if is_pt:
        ...
        if kernel1:
            cpt_0 = conv_unit1(pre_F * 3 + PT_F * isPT_F + P_N * (not isPT_F), conv_F, drop, H, W)(cpt_con1)
        else:
            cpt_0 = conv_unit0(pre_F * 3 + PT_F * isPT_F + P_N * (not isPT_F), conv_F, drop, H, W)(cpt_con1)

    else:
        cpt_con1 = Concatenate(axis=1)([c_out1, p_out1, t_out1])
        if kernel1:
            cpt_0 = conv_unit1(pre_F * 3, conv_F, drop, H, W)(cpt_con1)  # early fusion
        else:
            cpt_0 = conv_unit0(pre_F * 3, conv_F, drop, H, W)(cpt_con1)

    Res_layers = []
    if is_plus:
        for i in range(R_N):
            locals()['cpt_{}'.format(i + 1)] = \
                Res_plus('Res_plus_' + str(i + 1), conv_F, plus, rate, drop, H, W)(locals()['cpt_{}'.format(i)])
            Res_layers.append(locals()['cpt_{}'.format(i + 1)])
    else:
        for i in range(R_N):
            locals()['cpt_{}'.format(i + 1)] = \
                Res_normal('Res_normal_' + str(i + 1), conv_F, drop, H, W)(locals()['cpt_{}'.format(i)])
            Res_layers.append(locals()['cpt_{}'.format(i + 1)])

    if multi_scale_fusion:
        cpt_fusion = Concatenate(axis=1)(Res_layers)
    else:
        cpt_fusion = locals()['cpt_{}'.format(R_N)]

    cpt_conv2 = Activation('relu')(cpt_fusion)
    ...
    cpt_conv1 = Lambda(restore_layer)(cpt_conv1)
    ...
    DeepSTN_model.compile(loss=LOSS, optimizer=OPTIMIZER)
    ...
    print('***** multi scale fusion  : ', multi_scale_fusion)
