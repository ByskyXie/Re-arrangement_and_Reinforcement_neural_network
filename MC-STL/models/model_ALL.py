...
class Contrast_Enhance(nn.Module):
    def __init__(self, in_channels, out_channels,nb_residual_unit, patch_size, gru_dp,
        ...
        self.gru = nn.GRU(self.in_chans*self.map_height*self.map_width, self.in_chans*self.map_height*self.map_width, gru_dp, batch_first=True)
...
class Mask_Enhance(nn.Module):
    def __init__(self, in_channels, out_channels, nb_residual_unit, patch_size,
        ...
        self.conv = nn.Conv2d(1, self.len, 3, 1, 1)  # pre inchannels=int(self.in_chans/2), but unsqueeze is 1
        self.linear = nn.Linear(in_features=self.patch_num*self.embed_dim, out_features=map_width*map_height*self.in_chans, bias=True)
        ...
        self.end_conv = nn.Conv2d(4, out_channels, 3, 1, 1)
    def forward(self, x):
...
class ext(nn.Module):
    def __init__(self):
        ...
        self.embed_day = nn.Embedding(128, 10)
        self.embed_weekend = nn.Embedding(128, 10)
        self.embed_hour = nn.Embedding(128, 10)  # hour range [0, 23]
        self.embed_holiday = nn.Embedding(128, 10)
        self.embed_wind = nn.Embedding(128, 10)
        self.embed_weather = nn.Embedding(128, 10)  # ignore 0, thus use 18
...
class MC_STL(nn.Module):
    ...
    def forward(self, X, X_ext=None):
        ...
        if self.ext_flag == True:
            X_ext = self.xc_ext(X_ext).view(-1, x_len, self.in_chan, self.img_height, self.img_width)
            X += X_ext

        X = X.view(-1, x_len * channel, self.img_height, self.img_width)
