...
class MC_STL(nn.Module):
    ...
    def forward(self, X, X_ext=None):
        _, x_len, channel, _, _ = X.shape  # [128, 8, 1, 12, 12]

        if self.ext_flag == True:
            X_ext = self.xc_ext(X_ext).view(-1, x_len, self.in_chan, self.img_width, self.img_height)
            X += X_ext

        X = X.view(-1, x_len * channel, self.img_width, self.img_height)  # [128, 8, 1, 12, 12]-->[128, 8, 12, 12]
        x_zero = torch.zeros_like(X)
        X = torch.cat((x_zero, X), dim=-1)  # [128, 8, 12, 12*2]  # what are you doing????
