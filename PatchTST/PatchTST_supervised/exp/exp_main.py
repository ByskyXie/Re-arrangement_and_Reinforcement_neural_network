...
class Exp_Main(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        total_loss = []
        self.model.eval()
        avg_er, avg_nmae, avg_rmse, counter = 0, 0, 0, 0
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                avg_er += error_rate(outputs, batch_y).detach()
                avg_nmae += NMAE(outputs, batch_y).detach()
                avg_rmse += torch.sqrt(torch.pow(outputs - batch_y, 2).mean()).detach()
                counter += 1
        ...
        print(f'Val_ER=\t{avg_er / counter}\tVal_NMAE='
              f'\t{avg_nmae / counter}\tVal_RMSE=\t{avg_rmse / counter}')
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            print(f'Epoch:{epoch + 1}\ttrain_ER=\t{avg_er / counter}'
                  f'\ttrain_NMAE=\t{avg_nmae / counter}\ttrain_RMSE=\t{avg_rmse / counter}\ttime={time.time() - epoch_time}')
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                outputs, batch_y = torch.from_numpy(outputs), torch.from_numpy(batch_y)
                avg_er += error_rate(outputs, batch_y).detach()
                avg_nmae += NMAE(outputs, batch_y).detach()
                avg_rmse += torch.sqrt(torch.pow(outputs - batch_y, 2).mean()).detach()
                counter += 1
        ...
        print(f'Test\ttest_ER=\t{avg_er / counter}'
              f'\ttest_NMAE=\t{avg_nmae / counter}\ttest_RMSE=\t{avg_rmse / counter}')
        f = open("result.txt", 'a')
        f.write(setting + "  \n")
