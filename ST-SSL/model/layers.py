...
class TemporalHeteroModel(nn.Module):
    ...
    def forward(self, z1, z2):
        ...
        loss = self.b_xent(logits, self.lbl[:logits.shape[0]])
        return loss
...
class STEncoder(nn.Module):
    def __init__(self, Kt, Ks, blocks, input_length, num_nodes, droprate=0.1):
        ...
        self.Ks=Ks  # Ks:kernel_size of spatial?   Kt:kernel_size of temporal?
        c = blocks[0]  # [2, int(args.d_model//2), args.d_model]
        self.tconv11 = TemporalConvLayer(Kt, c[0], c[1], "GLU")
        self.pooler = Pooler(max(7, input_length - (Kt - 1)), c[1])
        ...
        c = blocks[1]  # [args.d_model, int(args.d_model//2), args.d_model]
        ...
        out_len = max(1, input_length - 2 * (Kt - 1) * len(blocks))
        self.out_conv = TemporalConvLayer(out_len, c[2], c[2], "GLU")
        self.ln3 = nn.LayerNorm([num_nodes, c[2]])
        self.dropout3 = nn.Dropout(droprate)
        self.receptive_field = input_length + Kt -1  # -1 //for input_len=8, so delete 'input_length + Kt -1'

    def forward(self, x0, graph):  # [B(8), Seq_len(35), 1024, 2]   [1024, 1024]
        ...
        ## ST block 1
        x = self.tconv11(x)                                 # [8, 32, 35, 1024]  nclv
        x, x_agg, self.t_sim_mx = self.pooler(x)            # [8, 32, 33, 1024]
        self.s_sim_mx = sim_global(x_agg, sim_type='cos')

        x = self.sconv12(x, Lk)                             # [8, 32, 33, 1024]  nclv
        x = self.tconv13(x)                                 # [8, 64, 31, 1024]
        x = self.dropout1(self.ln1(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))  # [8, 64, 31, 1024]

        ## ST block 2
        x = self.tconv21(x)                                 # [8, 32, 29, 1024]
        x = self.sconv22(x, Lk)                             # [8, 32, 29, 1024]
        x = self.tconv23(x)                                 # [8, 64, 27, 1024]
        x = self.dropout2(self.ln2(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))  # [8, 64, 27, 1024]

        ## out block
        x = self.out_conv(x)                                # [8, 64, 1, 1024]  ncl(=1)v
        x = self.dropout3(self.ln3(x.permute(0, 2, 3, 1)))  # [8, 1, 1024, 64]  nlvc
        return x  # nl(=1)vc
