...
class STSSL(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = STEncoder(Kt=3, Ks=3, blocks=[[args.feature_num, int(args.d_model//2), args.d_model], [args.d_model, int(args.d_model//2), args.d_model]],
        ...
        self.mae = masked_mae_loss(mask_value=args.mask_value)
        self.args = args
    ...
    def pred_loss(self, z1, z2, y_true, scaler):
        ...
        # plt.show()
        # plt.imshow(y_true[0][0].cpu().detach().numpy())
        # plt.show()

        # loss = self.args.yita * self.mae(y_pred[..., 0], y_true[..., 0]) + \
        #         (1 - self.args.yita) * self.mae(y_pred[..., 1], y_true[..., 1])  # only consider IN/OUT (2 channels)
