...
class MatrixDataset(Dataset):
    ...
    def __init__(self, path, all_batch_num, fn_get_traffic_matrix=None, predict_matrix_num=3, input_matrix_num=4
        ...
        # # 查看一个点时间序列自相关性
        # a = tms.transpose(1, 2, 0)  # transpose交换维度
        # for i in a:
        #     for j in i:
        #         plt.plot(autocorrelation(j.reshape(-1), 48), linewidth=0.5)
        # plt.show()
...
#                 self.large_dataset_flag = True
    ...
    def __getitem__(self, index):
        ...
        # if self.offset_sampling:  # 废弃。在获得tms时随机抛弃部分值更合理，这样不会混入已抛去的值
        #     offsets = (np.random.rand(len(indices))*2-1).round().astype(np.int32)  # 加减1内偏移
        #     indices = np.array(indices, dtype=np.int32)+offsets
        ...
        # head_valid_pos = (index+self.input_matrix_num-1)*(1+self.predict_matrix_num)+1  # 预测
        # tail_valid_pos = (index+self.input_matrix_num-1)*(1+self.predict_matrix_num)+1+self.predict_matrix_num # 预测
...
class MatrixFillDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        indices = list(range(head_matrix_pos, tail_matrix_pos))  # 均匀取值
        # if self.offset_sampling:  # 废弃。在获得tms时随机抛弃部分值更合理，这样不会混入已抛去的值
        #     offsets = (np.random.rand(len(indices))*2-1).round().astype(np.int32)  # 加减1内偏移
        #     indices = np.array(indices, dtype=np.int32)+offsets
        #     indices = indices.tolist()
...
def get_traffic_matrix_video(path, all_batch_size=1000, name: str = 'Video_matrix'):
    ...
    tms = np.stack(tms)  # [all_batch, w, h] 分别为 batch，行，列
    # # 行打乱
...
if __name__ == '__main__':
    ...
    mat, time_seq = get_traffic_matrix_taxibj("./dataset/TaxiBJ/BJ16_M32x32_T30_InOut.h5")
    print(mat.shape)
