...
def train_tim_model(input_matrix_num, predict_matrix_num, batch_size, in_channels, epochs, sampling_rate1, sampling_rate2):
    ...
    # for m in timNetwork.parameters():
    #     print(type(m), m.numel())

    # TODO:TaxiBj
    # timNetwork = MapInterpolationModel(input_matrix_num, predict_matrix_num, in_channels=in_channels, blocks_num=4
    #                               , matrix_row=32, matrix_column=32).to(device)
    # datasets = MatrixDataset(path='./dataset/TaxiBJ/BJ16_M32x32_T30_InOut.h5', all_batch_num=1250,
    #                          predict_matrix_num=predict_matrix_num, input_matrix_num=input_matrix_num,
    ...
    for epoch in range(epochs):
        ...
        for train, valid, time_seq in trainloader:
            ...
            # plot after reshape
            # train_ = train[0][0][0].detach().cpu().numpy()
            # plt.subplot(321)
            # plt.imshow(train_, interpolation='nearest', cmap='Blues', origin='lower')
            # plt.gca().invert_yaxis()
            # row, column = timNetwork.arranger.relin0.row_rearrange_matrix, timNetwork.arranger.relin0.column_rearrange_matrix
            # t1 = torch.matmul(row, train[0][0][0].detach()).detach().cpu().numpy()
            # t2 = torch.matmul(row, torch.matmul(train[0][0][0].detach(), column)).detach().cpu().numpy()
            # plt.subplot(323)
            # plt.imshow(t1, interpolation='nearest', cmap='Blues', origin='lower')
            # plt.gca().invert_yaxis()
            # plt.subplot(324)
            # plt.imshow(t2, interpolation='nearest', cmap='Blues', origin='lower')
            # plt.gca().invert_yaxis()
            # plt.subplot(325)
            # plt.imshow(timNetwork.arranger.relin0.row_rearrange_matrix.cpu().detach(), interpolation='nearest', cmap='binary', origin='lower')
            # plt.gca().invert_yaxis()
            # plt.subplot(326)
            # plt.imshow(timNetwork.arranger.relin0.column_rearrange_matrix.cpu().detach(), interpolation='nearest', cmap='binary', origin='lower')
            # plt.gca().invert_yaxis()
            # plt.show()
...
def train_pred_model(input_matrix_num, predict_matrix_num, batch_size, in_channels, epochs, sampling_rate1, sampling_rate2):
    ...
    # predModel = PredictionModel(input_matrix_num, predict_matrix_num, in_channels=in_channels, blocks_num=4
    #                               , matrix_row=32, matrix_column=32).to(device)
    # datasets = MatrixPredDataset(path='./dataset/TaxiBJ/BJ16_M32x32_T30_InOut.h5', all_batch_num=2000,
    #                          predict_matrix_num=predict_matrix_num, input_matrix_num=input_matrix_num,
    #                          fn_get_traffic_matrix=get_traffic_matrix_taxibj, gpu_mode=True,
    #                          sampling_rate1=sampling_rate1, sampling_rate2=sampling_rate2)
    ...
    for epoch in range(epochs):
        ...
        for train, valid, time_seq in trainloader:
            ...
            # plt.subplot(221)
            # plt.imshow(train_, interpolation='nearest', cmap='Blues', origin='lower')
            # row, column = predModel.arranger.relin0.row_rearrange_matrix, predModel.arranger.relin0.column_rearrange_matrix
            # t1 = torch.matmul(row, train[0][0][0].detach()).detach().cpu().numpy()
            # t2 = torch.matmul(row, torch.matmul(train[0][0][0].detach(), column)).detach().cpu().numpy()
            # plt.subplot(223)
            # plt.imshow(t1, interpolation='nearest', cmap='Blues', origin='lower')
...
def train_fill_model(input_matrix_num, predict_matrix_num, batch_size, in_channels, epochs, sampling_rate1, sampling_rate2):
    ...
    fillModel = FillModel(input_matrix_num, in_channels=in_channels, blocks_num=4
                                  , matrix_row=32, matrix_column=32).to(device)
    datasets = MatrixFillDataset(path='./dataset/TaxiBJ/BJ16_M32x32_T30_InOut.h5', all_batch_num=2000,
                             input_matrix_num=input_matrix_num,
                             fn_get_traffic_matrix=get_traffic_matrix_taxibj, gpu_mode=True,
                             sampling_rate1=sampling_rate1, sampling_rate2=sampling_rate2)
    ...
    for epoch in range(epochs):
        ...
        for train, valid, time_seq in trainloader:
            ...
            # plt.subplot(221)
            # plt.imshow(train_, interpolation='nearest', cmap='Blues', origin='lower')
            # row, column = fillModel.arranger.relin0.row_rearrange_matrix, fillModel.arranger.relin0.column_rearrange_matrix
            # t1 = torch.matmul(row, train[0][0][0].detach()).detach().cpu().numpy()
            # t2 = torch.matmul(row, torch.matmul(train[0][0][0].detach(), column)).detach().cpu().numpy()
            # plt.subplot(223)
            # plt.imshow(t1, interpolation='nearest', cmap='Blues', origin='lower')
